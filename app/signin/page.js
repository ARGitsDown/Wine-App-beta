...
export default async function SignInPage({ searchParams }) {
  ...
  // the owner sets the three variables, not a fault.
  if (!isAuthConfigured()) {
    return (
      <div className="mx-auto flex w-full max-w-md flex-col gap-4 px-4 py-16">
        ...
        <p className="text-sm text-zinc-500">
          ...
          <code className="text-xs">AUTH_GOOGLE_SECRET</code> — see{" "}
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col gap-6 px-4 py-16">
      ...
      {/* Auth.js reports refusals by redirecting back here with ?error, so
          the most likely failure - a real Google account that simply is not
          on the list - has to say so in words rather than leaving someone
          to conclude their password is wrong. */}
      {error && (
        <p
          role="alert"
        ...
        >
          {error === "AccessDenied"
            ? "That account hasn't been invited to this cellar. If you think it should have been, ask whoever runs it to add your address."
