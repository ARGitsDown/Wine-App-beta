...
export async function getBottles(status, filters = {}) {
  ...
  const { variety, region, subRegion, country, wineColor, vintage, rating } = filters;
  ...
  return withRating.filter((bottle) => {
    ...
    if (
      subRegion &&
      !bottle.subRegion?.toLowerCase().includes(subRegion.toLowerCase())
    ) {
      return false;
    }
    if (
    ...
    if (wineColor && bottle.wineColor !== wineColor) {
      return false;
    }
