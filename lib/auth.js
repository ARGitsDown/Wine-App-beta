...
// this half is only the lookups it needs, plus two side effects - an
// invite records when it was first used, so the list shows who has
// actually turned up rather than only who was asked.
//
// The second side effect is the one that matters more than it looks: the
// bootstrap path (an unclaimed cellar, no invite needed yet) only exists
// for the first sign-in. The moment it succeeds, the cellar is claimed and
// that path closes - permanently, by design. Someone who got in only
// through that path has no invite row, so with nothing else recorded, the
// *second* sign-in has neither an invite nor an unclaimed cellar to pass
// through, and is denied. That would lock the owner out of their own app
// after exactly one session. So a bootstrap-path pass writes the invite
// that a normal invited sign-in would already have, making every sign-in
// after the first behave identically regardless of how the first one got
// in.
async function isAllowedToSignIn(email) {
  ...
  if (invite) {
    ...
    // No invite row can only mean this pass came through the bootstrap
    // path - write the invite it never had, already accepted.
    await prisma.invite.create({
      data: {
        email: address,
        note: "Claimed the seeded cellar directly - no invite was sent.",
        acceptedAt: new Date(),
      },
    });
  }
