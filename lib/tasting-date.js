...
// Today, anchored the same way a picked date is. Used where the app stamps
// a date on your behalf (pressing "Tasted"), so every stored value has one
// shape rather than raw timestamps from buttons and noon from the editor -
// which would otherwise sort and compare inconsistently.
export function todayAtNoonUtc() {
  return parseTastedDate(todayInputValue());
}
