...
// Whose cellar /guest shows. A guest browsing at /guest has no session at
// all - there is nothing for lib/scoped-prisma.js's extension to read - so
// this can't be currentOwnerId's job. It also isn't really "the current
// owner" in the same sense: a guest link isn't per-invitee (see Phase 4 in
// FUTURE_CAPABILITIES.md for what that would even mean), it's the one
// friend/family view of the one cellar this feature was built for. Until
// that phase exists, "the earliest-created owner" is the same answer
// currentOwnerId gives before auth is configured at all - both describe
// the single cellar this app started as.
export async function guestOwnerId() {
  const owner = await prisma.user.findFirst({ orderBy: { createdAt: "asc" } });
  if (!owner) {
    throw new Error(
      "No owner row found. The 20260921170000_add_owner migration seeds one - run `prisma migrate deploy`."
